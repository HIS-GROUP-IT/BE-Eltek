...
public async createEmployee(employeeData: Partial<IEmployee>): Promise<IEmployee> {
    ...
    if (existingEmployee) {
        if (existingEmployee.email === employeeData.email) {
            throw new HttpException(400, "Employee with this email already exists");
        }
        if (existingEmployee.idNumber === employeeData.idNumber) {
            throw new HttpException(400, "Employee with this ID number already exists");
        }
    }
    ...
    public async deleteEmployee(employeeId: number): Promise<void> {
      ...
      try {
          ...
          throw error instanceof HttpException ? error : new HttpException(500, "Error deleting employee");
      }
  }
    ...
    public async assignEmployeesToProject(assignedEmployees: IAssignedEmployees[]): Promise<void> {
      ...
      const newAssignments = assignedEmployees.filter(e => !alreadyAssignedIds.includes(e.employeeId));
  
      if (newAssignments.length === 0) {
          throw new HttpException(400, "All employees are already assigned to this project");
      }
  
      const transaction: Transaction = await EmployeeProject.sequelize!.transaction();
